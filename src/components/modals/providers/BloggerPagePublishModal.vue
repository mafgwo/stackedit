  ...
  <modal-inner aria-label="发布到Blogger Page">
    <div class="modal__content">
      ...
      <form-entry label="Blog URL" error="blogUrl">
        <template v-slot:field><input class="textfield" type="text" v-model.trim="blogUrl" @keydown.enter="resolve()"></template>
        <div class="form-entry__info">
          <b>例如:</b> http://example.blogger.com/
        </div>
      </form-entry>
      <form-entry label="现有页面ID" info="可选的">
        <template v-slot:field><input class="textfield" type="text" v-model.trim="pageId" @keydown.enter="resolve()"></template>
      </form-entry>
      <form-entry label="模板">
