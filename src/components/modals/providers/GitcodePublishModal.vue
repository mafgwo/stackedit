<template>
  <modal-inner aria-label="发布到GitCode">
    <div class="modal__content">
      <div class="modal__image">
        <icon-provider provider-id="gitcode"></icon-provider>
      </div>
      <p>发布 <b>{{currentFileName}}</b> 到您的 <b>GitCode</b> 仓库.</p>
      <form-entry label="仓库URL" error="repoUrl">
        ...
        <div class="form-entry__info">
          <b>例如:</b> https://gitcode.com/owner/my-repo
        </div>
      </form-entry>
...
export default modalTemplate({
  ...
  computedLocalSettings: {
    repoUrl: 'gitcodeRepoUrl',
    selectedTemplate: 'gitcodePublishTemplate',
  },
  created() {
  ...
  methods: {
    resolve() {
      ...
      if (parsedRepo && this.path) {
        const location = gitcodeProvider.makeLocation(
          this.config.token,
          parsedRepo.owner,
          parsedRepo.repo,
          this.branch || 'main',
